...
function email.init()
	email.inboxes = colddb.Colddb("email")
...
function email.get_inbox(name)
	return email.inboxes.get(name) or {}
end

function email.clear_inbox(name)
	email.inboxes.remove(name)
end
...
function email.send_mail(name, to, msg)
	...
	if true then
		...
		if email.inboxes.get_key(to) then
			local data = email.inboxes.get(to)
			table.insert(data, mail)
			email.inboxes.set(to, data)
		else
			email.inboxes.set(to, mail)
		end
